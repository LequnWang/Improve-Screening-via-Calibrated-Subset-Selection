"""
Select a Shortlist of Applicants Based on Calibrated Subset Selection Algorithm for Each Group
"""
import argparse
import pickle
import numpy as np
...
if __name__ == "__main__":
    ...
    t_hat_maj = find_t_hat(k=k_maj, m=m_maj, n=n_maj, alpha=alpha, scores_cal=scores_cal_maj, y_cal=y_cal_maj)
    t_hat_min = find_t_hat(k=k_min, m=m_min, n=n_min, alpha=alpha, scores_cal=scores_cal_min, y_cal=y_cal_min)
    ...
    s_test_raw_maj = shortlist(t_hat_maj, scores_test_raw_maj)
    s_test_raw_min = shortlist(t_hat_min, scores_test_raw_min)
    performance_metrics = {}
    s_cal_maj = shortlist(t_hat_maj, scores_cal_maj)
    s_cal_min = shortlist(t_hat_min, scores_cal_min)
